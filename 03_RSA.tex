...
Formally it is a a set of propositions (sets of
  worlds) all of which are taken for granted. The context set $C$, as defined above, can be thought of as the the conjunction of all of the propositions that are being taken for granted:
$$
C = \bigcap \mathit{CG}
$$ 
In our example scenario, Alice (the speaker) could reasonably take for granted certain propositions representing plausible observations about John's smoking habits --- whether he smoked in the past, and whether he does not. 
The CG prior is built around the assumption that each of these observations is equally likely \emph{a priori}. For concreteness, we assign each observation probability $.4$.
\begin{equation}
P(C) \propto  P(\text{obs}) \times \delta_{C \Rightarrow \text{obs}}
%\Pr(C)=0.95\cdot\Pr(C=\cap\text{CG}_\text{Obs})+0.05\cdot 1/15 
\label{eq:CGprior}
\end{equation}
$\delta_{C \Rightarrow \text{obs}}$ is a delta function returning 1 for context sets which entail the observation, and 0 for all others.
Our prior is generated by adding a small amount of noise to this model, so that every non-empty $C$ has nonzero prior probability. This model assigns low prior probability to the context sets that cannot be built up via Boolean combinations of these natural observations. One example of a context set receiving low prior probability in this model is\verb=change=, the rather complex assumption that John has \emph{either} switched from smoking to not, \emph{or} switched from not smoking to smoking --- but we do not know which. In contrast, context sets such as \emph{John used to smoke} and \emph{John did not smoke and does not smoke now} receive higher probability because they correspond to observations that the speaker could plausibly have made.  
